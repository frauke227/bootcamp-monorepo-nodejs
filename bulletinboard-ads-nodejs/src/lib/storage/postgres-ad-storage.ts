...
export default class PostgresAdStorage {
  ...
  static READ = 'SELECT ads.id, ads.title, ads.contact, ads.price, ads.currency, COALESCE(reviews.review, 0) AS "averageContactRating" FROM ads LEFT JOIN reviews USING (contact) WHERE id = $1'
  static READ_ALL = 'SELECT id, title, contact, price, currency, COALESCE(reviews.review, 0) AS "averageContactRating" FROM ads LEFT JOIN reviews USING (contact)'
